...
export async function moveNodeToPosition(node: Node, option: MoveNodeToPositionOption) {
    ...
    option.curveFunc = option.curveFunc || ((option: CurveFuncOption) => {
        const moveX = (option.targetPosition.x - originPosition.x) / option.timesAmount
        const moveY = (option.targetPosition.y - originPosition.y) / option.timesAmount
    ...
    return new Promise<void>(res => {
        ...
        const inter = setInterval(() => {
            ...
            nextPosition = option.curveFunc({
              times, timesAmount, originPosition, targetPosition: option.targetPosition, currentPosition: {x: node.position.x, y: node.position.y}
            })
